...
void main(void)
{
  ...
  //state2 = 0;
	//speed_right = -BLANA;
	//speed_left = -BLANA;
	//ramp(&speed_right, &speed_left, ms, &state2);
	//ramp(&speed_right, &speed_left, ms);
...
inline void idle(void)
{
	...
	if (no_enemy == SPIN)
	{
		...
		{
			spin = (default_dir == RIGHT) ? -SPIN_CONST:SPIN_CONST;
		}
//		else
//		{
//			no_enemy = SEEK;
